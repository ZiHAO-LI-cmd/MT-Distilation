...
class InferenceDataset(Dataset):
    def __init__(self, source_texts, tokenizer, max_length=200, prefix=">>ukr<< "): # replace "ukr" with target language code
        self.source_texts = [prefix + text for text in source_texts]
...
def create_inference_dataloader(source_texts, tokenizer, batch_size=32, max_length=200, prefix=">>ukr<< "): # replace "ukr" with target language code
    dataset = InferenceDataset(source_texts, tokenizer, max_length, prefix)
...
def translate(args):
    ...
    inference_dataloader = create_inference_dataloader(source_texts, tokenizer, batch_size=args.batch_size, prefix=args.prefix)
    translated_texts = []
    batch_count = 0
...
def main():
    ...
    parser.add_argument('--prefix', type=str, help='Prefix to add to each sentence')
